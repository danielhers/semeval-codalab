...
if __name__ == "__main__":
    ...
    #input_dir = '/cs/usr/zohara/PycharmProjects/semeval-ucca/submission'
    #output_dir = "/cs/usr/zohara/PycharmProjects/semeval-ucca/test_data"
        ...
        with open(os.path.join(output_dir, 'scores.html'), 'w') as output_html_file:
            for (lang, track) in competitions:
                ...
                if os.path.exists(submission_dir+"/"+competition):
                    files = [[os.path.join(d, f) for f in os.listdir(d)] if os.path.isdir(d) else [d] for d in (submission_dir+"/"+competition, truth_dir+"/"+competition)]#args.guessed, args.ref)]
                    ...
                    output_file.write("%s_UCCA_averaged_labeled_f1: %.3f\n" % (competition, summary.average_f1(LABELED)))
                    output_html_file.write("######################\n")
                    output_html_file.write("%s_UCCA_averaged_labeled_f1: %.3f\n" % (competition, summary.average_f1(LABELED)))
                    ...
                    output_file.write("%s_UCCA_averaged_unlabeled_f1: %.3f\n" % (competition, summary.average_f1(UNLABELED)))
                    output_html_file.write("######################\n")
                    output_html_file.write("%s_UCCA_averaged_unlabeled_f1: %.3f\n" % (competition, summary.average_f1(UNLABELED)))
